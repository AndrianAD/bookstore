...
export default function CartDrawer({ open, onClose }: CartDrawerProps) {
  ...
  const formatPhone = (v: string) => {
    ...
    if (digits.startsWith('380')) {
      ...
      const parts = [d.slice(0, 3), d.slice(3, 5), d.slice(5, 8), d.slice(8, 10), d.slice(10, 12)]
        .filter(Boolean)
      return '+' + parts.join(' ')
    }
    return '+' + digits
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div
        className={`fixed top-0 right-0 bottom-0 w-full sm:w-[440px] bg-white z-50 shadow-2xl
            ...
            <div className="flex-1 overflow-y-auto p-5">
              {items.length === 0 ? (
                ...
                <div className="space-y-3">
                  {items.map((item) => (
                    <div key={item.id} className="flex gap-3 p-3 rounded-xl border border-gray-100 hover:border-gray-200 transition-colors">
                      {/* Image */}
                      <div className="relative w-14 h-20 rounded-lg overflow-hidden bg-gradient-to-br from-accent-100 to-accent-200 shrink-0">
                        {item.image_url ? (
                        ...
                        <div className="mt-auto flex items-center justify-between">
                          <div className="flex items-center gap-1 bg-gray-100 rounded-lg">
                            <button
                              ...
                              className="w-7 h-7 rounded-lg text-gray-600 hover:bg-gray-200 transition-colors flex items-center justify-center"
                            >
                              −
                            </button>
            ...
            {items.length > 0 && (
              <div className="border-t border-gray-100 p-5 space-y-4 shrink-0">
                ...
                <p className="text-xs text-gray-400 text-center">
                  Оставьте номер — оператор перезвонит для подтверждения
                </p>
              </div>
            )}
          </>
          ...
          <form onSubmit={handleSubmit} className="flex-1 flex flex-col">
            <div className="flex-1 overflow-y-auto p-5 space-y-5">
              ...
              <div className="bg-gray-50 rounded-2xl p-4">
                <h3 className="font-semibold text-sm text-gray-700 mb-3">Ваш заказ</h3>
            ...
            <div className="border-t border-gray-100 p-5 shrink-0">
              <button
                type="submit"
